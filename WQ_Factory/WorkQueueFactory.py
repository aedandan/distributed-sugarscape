...
if __name__ == "__main__":
    ...
    if batch_type == slurm_batch_type:
        subprocess.run(["work_queue_factory", "-T", batch_type, "-B", "--partition=skx -t 0:20:00", "-M", manager_name, "-w", "10", "-W", "50", "--workers-per-cycle", "10"])
    elif batch_type == condor_batch_type:
        subprocess.run(["work_queue_factory", "-T", batch_type, "-M", manager_name, "-w", "10", "-W", "50", "--workers-per-cycle", "10"])
    else:
